...
import { Settings, TrendingUp, Package, Info, DollarSign, Calculator, Link as LinkIcon, Plus, Trash2, ChevronDown, ChevronUp, Image as ImageIcon, Check, Save, History, ExternalLink, Download, X, Copy } from 'lucide-react';
...
export default function App() {
  ...
  const exportHistoryToCSV = () => {
    ...
    // CSV 头部
    const headers = ['产品名称', '货源链接', '图片链接', '保存时间'];
    
    // CSV 数据行
    const rows = sourceHistory.map(item => [
      `"${(item.name || '').replace(/"/g, '""')}"`,
      `"${(item.link || '').replace(/"/g, '""')}"`,
    ...
    link.setAttribute('download', `tiktok_source_history_${new Date().toISOString().split('T')[0]}.csv`);
  ...
  return (
    <div className="min-h-screen p-6 max-w-[1240px] mx-auto flex flex-col gap-6 select-none bg-[#f1f5f9]">
          ...
          <button 
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-xl font-bold text-xs transition-all shadow-lg ${
              saveStatus === 'success' 
                ...
                : 'bg-ink text-white hover:bg-slate-800 shadow-slate-900/20 active:scale-95'
            }`}
          >
            {saveStatus === 'saving' ? (
            ...
            {saveStatus === 'saving' ? '保存中...' : saveStatus === 'success' ? '已保存本地' : '保存当前设置'}
          </button>
          <div className="hidden md:flex flex-col items-end">
            <div className="text-[11px] font-bold text-muted flex items-center gap-1.5 uppercase tracking-wider">
              实时汇率同步：<span className="text-blue-600">已开启</span>
            </div>
            <div className="text-[10px] text-muted/60 mt-0.5 font-mono">北京时间: {new Date().toLocaleDateString('zh-CN')}</div>
          </div>
        </div>
      </header>
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <input 
                  ...
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl pl-10 pr-4 py-2.5 text-sm font-medium focus:ring-2 focus:ring-blue-500/10 focus:border-blue-500 outline-none transition-all"
                />
                  ...
                  <div className="absolute left-0 right-0 top-full mt-2 bg-white border border-slate-200 rounded-xl shadow-xl z-50 max-h-[240px] overflow-y-auto overflow-x-hidden p-2 space-y-1 animate-in fade-in slide-in-from-top-2 duration-200">
                    <div className="flex justify-between items-center px-2 py-1 border-b border-slate-100 mb-1">
                      <div className="flex items-center gap-2">
                        <span className="text-[10px] font-bold text-muted uppercase">最近历史 ({sourceHistory.length}/500)</span>
                      </div>
                      <div className="flex items-center gap-3">
                        <button 
                          onClick={(e) => { e.stopPropagation(); exportHistoryToCSV(); }}
                          className="text-[10px] text-blue-600 hover:text-blue-700 font-bold flex items-center gap-1"
                        >
                          <Download className="w-2.5 h-2.5" /> 导出表格
                        </button>
                        <button 
                          onClick={(e) => { e.stopPropagation(); if (confirm('确认清空所有 500 条记录吗？')) { setSourceHistory([]); localStorage.removeItem('sourceHistory'); } }}
                      ...
                      >
                        ...
                        <div className="flex-grow min-w-0">
                          ...
                          <div className="flex items-center gap-1 mt-0.5">
                            ...
                            <ExternalLink className="w-2.5 h-2.5 text-slate-300 group-hover:text-blue-400" />
                          </div>
                        </div>
                        <button 
                ...
                {country.mode === 'backward' ? (
                  <>
                    <div className="metric-row !border-slate-300">
                      <span className="text-ink font-bold">前端定价</span>
                      <div className="flex items-center gap-1 font-mono font-bold text-blue-600">
                        <input 
                          ...
                          className="w-20 text-right bg-blue-50/50 rounded px-1 outline-none"
                        />
                  ...
                  <>
                    ...
                    <div className="mt-auto bg-blue-600 rounded-xl p-4 text-center shadow-lg shadow-blue-500/20">
                      <div className="text-[9px] uppercase font-bold text-blue-100 mb-1 tracking-wider opacity-70">建议前端售价</div>
